...
class DescreteSequenceDataset(Dataset):
    def __init__(
        self, data: pd.DataFrame, duration: int = 1440, step_size: int = 10
    ):
        """Torch Dataset for descretised sequence data.

        Args:
            data (DataFrame): Population of sequences.
            duration (int): Length of desired sequences in minutes.
            step_size (int): Step size of descretisation in minutes.
        """
        self.duration = duration
        self.step_size = step_size
        self.steps = duration // step_size
        self.size = data.pid.nunique()
...
def descretise_population(
    ...
    # todo: we keep the channel dimension so we look like an image, remove?
